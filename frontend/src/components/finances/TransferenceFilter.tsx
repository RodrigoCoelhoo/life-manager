...
interface TransferenceFilterProps {
	filters: TransferenceFilters;
	onApplyFilters: (newFilters: TransferenceFilters) => void;
	onClose: () => void;
}

export default function TransferenceFilter({ filters, onApplyFilters, onClose }: TransferenceFilterProps) {
	...
	const [tempFilters, setTempFilters] = useState<TransferenceFilters>({ ...filters });

	const startDateRef = useRef<any>(null);
	const endDateRef = useRef<any>(null);

	const updateTempFilter = <K extends keyof TransferenceFilters>(
		key: K,
		value: TransferenceFilters[K]
	) => {
		setTempFilters(prev => ({
			...prev,
			[key]: value || undefined,
			...
			>
				...
				<form className="flex flex-col space-y-4 w-90 sm:w-110" onSubmit={handleSubmit}>
					...
					<div>
						...
						<div className="flex flex-row gap-2">
							<ButtonField
								value={tempFilters.receiver !== undefined ? tempFilters.receiver.name : ""}
								onClick={() => {
									setOpenWalletSearch(true);
									setFocusWallet("Receiver");
								}}
								icon={<MagnifyingGlassIcon width={16} height={16} />}
								placeholder="Select a wallet"
							/>

							{tempFilters.receiver !== undefined &&
								<div className="w-1/10 text-xl flex items-center justify-center hover:bg-background cursor-pointer rounded-xl">
									<MdClear
										onClick={() => updateTempFilter("receiver", undefined)}
									/>
								</div>}
						</div>
					</div>
			...
			<Modal isOpen={openWalletSearch} onClose={() => setOpenWalletSearch(false)}>
				<SearchList<WalletResponseDTO>
					...
					onSelect={(wallet: WalletResponseDTO) => {
						if (focusWallet === "Sender") {
							updateTempFilter("sender", wallet);
						} else if (focusWallet === "Receiver") {
							updateTempFilter("receiver", wallet);
						}
